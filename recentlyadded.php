...
$qstring = "";
if ($mode == "random") {
	$qstring = "SELECT Uri FROM Tracktable WHERE DATE_SUB(CURDATE(),INTERVAL 30 DAY) <= DateAdded ORDER BY RAND()";
} else {
	// This rather cumbersome query gives us albums in a random order but tracks in the correct order.
	$qstring = "SELECT Uri FROM (SELECT DISTINCT Albumindex FROM Tracktable WHERE DATE_SUB(CURDATE(),INTERVAL 30 DAY) <= DateAdded ORDER BY RAND()) as x JOIN (SELECT Uri, Albumindex FROM Tracktable ORDER BY Disc, Trackno) AS y USING (Albumindex)";
}
